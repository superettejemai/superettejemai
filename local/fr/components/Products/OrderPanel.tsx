...
export default function OrderPanel({ cartItems, onCartUpdate, onCheckoutSuccess, currentSessionName }: OrderPanelProps) {
  ...
  const handleBarcodeScanned = async (barcode: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/products?barcode=${encodeURIComponent(barcode)}`, {
        headers: {
                ...
                <div className="text-center">
                  ...
                  <div className="flex space-x-2">
                    <button
                      ...
                      className="flex-1 py-4 bg-red-600 hover:bg-red-700 text-white text-sm font-medium transition-colors"
                    >
                      Annuler ({countdown}s)
                    </button>
                    <button
