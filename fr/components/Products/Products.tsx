...
export default function Products({ onAddToCart }: ProductsProps) {
  ...
  const fetchProducts = useCallback(async (searchQuery: string = '') => {
    try {
      ...
      const response = await fetch(`https://superettejemai.onrender.com/api/products?${params.toString()}`, {
        headers: {
        ...
        <div className="flex flex-col xs:flex-row items-start xs:items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-1 sm:gap-2 w-full">
            <button 
              ...
              className="px-2 sm:px-4 py-4  border border-gray-400 text-sm hover:bg-gray-100 flex items-center justify-center shrink-0 active:bg-gray-200 transition-colors"
            >
            ...
            >
              {categories.map((category) => (
                <button
                  ...
                  className={`px-2 sm:px-4 py-1 sm:py-4 capitalize text-xs sm:text-sm min-w-[4rem] sm:min-w-[6rem] whitespace-nowrap shrink-0 transition-colors border ${
